...
switch (menu_index) {
    ...
    if ( up_key[GAMEPAD_1] || up_key[GAMEPAD_2] || up_key[KEYBOARD] ) {
       ...
       option--;
       if (option < 0) then option = maxOption;
       Play_Sound(snd_choose,1,.5,0,5000,true,false);
       scale = 1;
   } else if ( down_key[GAMEPAD_1] || down_key[GAMEPAD_2] || down_key[KEYBOARD] ) {
       alarm[0] = room_speed*.2;
       option++;
       if (option > maxOption) then option = 0;
       Play_Sound(snd_choose,1,.5,0,5000,true,false);
       scale = 1;
    }
}
...
draw_text(baseX,baseY+40+halfMYY-credits_scroll,"Diogo Braga"); 
draw_text(baseX,baseY+48+halfMYY-credits_scroll,"Carlos Vila"); //Mike Renevo
draw_text(baseX,baseY+64+halfMYY-credits_scroll,"April Poland");
draw_set_color(c_gray);        
//draw_text(baseX,baseY+42+halfMYY-credits_scroll,"Lead Developers");
draw_text(baseX,baseY+56+halfMYY-credits_scroll,"Lead Developers");//Developer
...
//Text
draw_text(xxxx,yyyy[0],text_gameplay[0] );
draw_text(xxxx,yyyy[1],text_gameplay[1] );
draw_text(xxxx,yyyy[2],text_gameplay[2] );
draw_text(xxxx,yyyy[3],text_gameplay[3] );
draw_text(xxxx,yyyy[4],text_gameplay[4] );
draw_text(xxxx,yyyy[5],text_gameplay[5]);
draw_text(xxxx,yyyy[6],text_gameplay[6]);
draw_text(xxxx,yyyy[7],text_gameplay[7]);
draw_text(xxxx,yyyy[8],text_gameplay[8]);

//Select
if ((!up_key[GAMEPAD_1] || !up_key[GAMEPAD_2] || !up_key[KEYBOARD] || !down_key[0] || !down_key[1] || !down_key[KEYBOARD]) && alarm[0] <= room_speed*.1) {
    draw_text_colour(xxxx+1,yyyy[option]-1,text_gameplay[option],make_colour_rgb(237,45,80),make_colour_rgb(237,45,80),make_colour_rgb(237,45,80),make_colour_rgb(237,45,80),1);
    } else {
        draw_text_colour(xxxx,yyyy[option],text_gameplay[option],make_colour_rgb(237,45,80),make_colour_rgb(237,45,80),make_colour_rgb(237,45,80),make_colour_rgb(237,45,80),1);
}
            
//Option selection
draw_text(xxx,yyyy[0],vsync[global.screen_shake]);
draw_text(xxx,yyyy[1],vsync[global.flash]);
draw_text(xxx,yyyy[2],vsync[global.autocast]);
draw_text(xxx,yyyy[3],vsync[global.autopick]);
draw_text(xxx,yyyy[4],vsync[global.glow]);
draw_text(xxx,yyyy[5],vsync[global.hpbar]);
draw_text(xxx,yyyy[6],vsync[global.mapback]);
draw_text(xxx,yyyy[7],vsync[global.smooth_camera]);
...
switch (option) {
    ...
    if (alarm[2]<=0) {
        if (max(left_key[GAMEPAD_1],left_key[GAMEPAD_2],left_key[KEYBOARD],right_key[0],right_key[1],right_key[KEYBOARD])) {
            alarm[2] = room_speed*0.25;
            global.autocast = !global.autocast;
        }
    }
    break;
    ...
    if (alarm[2]<=0) {
        if (max(left_key[GAMEPAD_1],left_key[GAMEPAD_2],left_key[KEYBOARD],right_key[0],right_key[1],right_key[KEYBOARD])){
            alarm[2] = room_speed*0.25;
             global.autopick = !global.autopick;
        }
    }
    break;
    ...
    if (alarm[2]<=0) {
        if (max(left_key[GAMEPAD_1],left_key[GAMEPAD_2],left_key[KEYBOARD],right_key[0],right_key[1],right_key[KEYBOARD])){
            alarm[2] = room_speed*0.25;
            global.mapback = !global.mapback;
        }
    }
    break;
    
    case 7://Smooth camera
    if (alarm[2]<=0) {
        if (max(left_key[GAMEPAD_1],left_key[GAMEPAD_2],left_key[KEYBOARD],right_key[0],right_key[1],right_key[KEYBOARD])){
            alarm[2] = room_speed*0.25;
